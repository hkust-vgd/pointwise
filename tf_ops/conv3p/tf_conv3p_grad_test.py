...
def conv3p(points_tensor, input_tensor, kernel_tensor, voxel_size):
    return conv3p_module.conv3p(points_tensor, input_tensor, kernel_tensor, voxel_size);
...
def _conv3p_grad(op, grad_from_next_layer):
    ...
    input = op.inputs[1]
    filter = op.inputs[2]
    voxel_size = op.inputs[3]

    input_grad, filter_grad = conv3p_module.conv3p_grad(grad_from_next_layer, points, input, filter, voxel_size)
    return [None, input_grad, filter_grad, None]
...
class Conv3pTest(tf.test.TestCase):
  ...
  def test_grad(self):
    ...
    with self.test_session():
      ...
      output_tensor = conv3p(points_tensor, input_tensor, kernel_tensor, voxel_size_tensor);
